...
case "$ACTION" in
  "start")
    ...
    if [[ -n "$SAFE_MESSAGE" ]] && command -v brokkr-mem >/dev/null 2>&1; then
      # Pass sanitized message as argument (safe from shell injection)
      brokkr-mem recall "$SAFE_MESSAGE" --limit 10 2>/dev/null || true
    fi
    ;;
  ...
  "end")
    ...
    if [[ -n "$SAFE_MESSAGE" ]] && command -v brokkr-mem >/dev/null 2>&1; then
      # Pass sanitized message as argument (safe from shell injection)
      brokkr-mem session-style "$SAFE_MESSAGE" --save 2>/dev/null || true
    fi
    ;;
    
  *)
    echo "Usage: $0 [start|end]" >&2
    echo "Sanitizes MESSAGE environment variable and calls appropriate brokkr-mem function" >&2
    exit 1
    ;;
esac